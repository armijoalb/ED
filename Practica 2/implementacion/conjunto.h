...
private:
 vector<crimen> vc; // vector ORDENADO por crimen.id que almacena los elementos del conjunto
	...
	@brief Busca una ID mediante búsqueda binaria ya que es un conjunto ordenado.

	@return Devuelve un par. Si lo ha encontrado el segundo elemento (bool) es true y el primero (long int) es la posición del cojunto::entrada.
	Si el segundo elemento (bool) es false, el primero es la posición donde se insertaría dicho elemento.

	*/
	pair< vector<conjunto::entrada>::iterator ,bool> binarySearchID( const long int id ) ;
