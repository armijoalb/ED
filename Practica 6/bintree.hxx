...
/*____________________________________________________________ */
template <typename T>
void bintree<T>::swap( bintree<T> & dest ){
  bintree<T> aux;

  aux.assign_subtree( *this, laraiz );
...
template <typename T>
void bintree<T>::setRoot( const typename bintree<T>::node & n ){
  copy(laraiz,n);
}
...
// Método begin_reverse().
template <typename T>
typename bintree<T>::inorder_iterator bintree<T>::begin_reverse(){
  // Nos posicionamos en la derecha.
  node n(laraiz);

  // Si la ráiz no es nula.
  if (!n.null()) // No movemos hasta el nodo más a la derecha.
    while (!n.right().null())
      n = n.right();
...
// Método end_reverse().
template <typename T>
typename bintree<T>::inorder_iterator bintree<T>::end_reverse(){
  // Devolvemos un nodo iterador con un nodo nulo.
  return bintree<T>::inorder_iterator(typename bintree<T>::node() );
}
...
template <typename T>
inline
bintree<T>::const_inorder_iterator::const_inorder_iterator( const inorder_iterator & i ){
  elnodo = i.elnodo;
}
...
// Método begin_reverse.
template <typename T>
typename bintree<T>::const_inorder_iterator bintree<T>::begin_reverse() const{
  node n(laraiz);
...
// Método end_reverse.
template <typename T>
typename bintree<T>::const_inorder_iterator bintree<T>::end_reverse() const{
  return bintree<T>::const_inorder_iterator( bintree<T>::node() );
}
